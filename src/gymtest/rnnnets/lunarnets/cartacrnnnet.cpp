...
std::vector<torch::Tensor> CartACRNNFcNet::forward(torch::Tensor input, std::vector<long> seqInput) {
...
std::vector<torch::Tensor> CartACRNNFcNet::forward(torch::Tensor input, torch::Tensor shuffleIndex, std::vector<long> seqInput) {
//	std::cout << "Input " << input.sizes() << std::endl;
	//input: {sum(seqLen), others}
	std::vector<torch::Tensor> splitVec;
	int index = 0;
...
//	states[0] = std::get<1>(rnnOutput);
	...
	//{step, batch, hidden} -> {step * batch, hidden}
	torch::Tensor fcInput = std::get<0>(rnnOutput).data(); //{sum(seqLen), others}
	fcInput = torch::index_select(fcInput, 0, shuffleIndex);
...
void CartACRNNFcNet::resetHState(const int index, std::vector<torch::Tensor>& states) {
