...
AirACHOGRUSlimNet::AirACHOGRUSlimNet(int aNum):
	...
	gru0(torch::nn::GRUOptions(3136, 512).batch_first(true)),
//	afc(3136, 512),
//	vfc(3136, 512),
	aOut(512, aNum),
	vOut(512, 1),
...
std::vector<torch::Tensor> AirACHOGRUSlimNet::forward(torch::Tensor input, std::vector<torch::Tensor>& states) {
...
//
//std::vector<torch::Tensor> AirACHOGRUSlimNet::forward(torch::Tensor input, int batchSize, int seqLen, std::vector<torch::Tensor> states) {
...
////	std::cout << "conv output " << output.sizes() << std::endl;
//	output = output.view({input.sizes()[0], -1}).view({batchSize, seqLen, -1});
//	std::cout << "conv output view " << output.sizes() << std::endl;
//
...
std::vector<torch::Tensor> AirACHOGRUSlimNet::forward(torch::Tensor input, int batchSize, int seqLen, std::vector<torch::Tensor> states, torch::Device deviceType) {
	//input {batch * seqLen, others}
//	std::cout << "--------------------------------> batch input " << input.sizes() << std::endl;
...
std::vector<torch::Tensor> AirACHOGRUSlimNet::createHStates(const int envNum, torch::Device deviceType) {
	torch::Tensor hState = torch::zeros({1, envNum, 512}).to(deviceType); //layernum, envNum, hiddenNum

	return {hState};
}

void AirACHOGRUSlimNet::resetHState(const int index, std::vector<torch::Tensor>& states) {
	states[0][0][index].fill_(0);
}

//Return independent tensor
//states: cellNum * {layerNum, batchNum, hiddenNum}
std::vector<torch::Tensor> AirACHOGRUSlimNet::getHState(const int envIndex, std::vector<torch::Tensor>& states) {
