...
AirACHOGRUNet::AirACHOGRUNet(int aNum, int hidden):
...
std::vector<torch::Tensor> AirACHOGRUNet::forward(torch::Tensor input, std::vector<torch::Tensor>& states) {
...
std::vector<torch::Tensor> AirACHOGRUNet::forward(torch::Tensor input, std::vector<long> seqInput, std::vector<torch::Tensor> states) {
...
std::pair<std::vector<torch::Tensor>, std::vector<torch::Tensor>> AirACHOGRUNet::forwardNext(
...
std::vector<torch::Tensor> AirACHOGRUNet::forwardNext(
...
std::vector<torch::Tensor> AirACHOGRUNet::createHStates(const int envNum, torch::Device deviceType) {
	torch::Tensor hState = torch::zeros({1, envNum, hiddenNum}).to(deviceType); //layernum, envNum, hiddenNum

	return {hState};
}

void AirACHOGRUNet::resetHState(const int index, std::vector<torch::Tensor>& states) {
	states[0][0][index].fill_(0);
}

//Return independent tensor
//states: cellNum * {layerNum, batchNum, hiddenNum}
std::vector<torch::Tensor> AirACHOGRUNet::getHState(const int envIndex, std::vector<torch::Tensor>& states) {
