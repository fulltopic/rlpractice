...
#ifndef INC_ALG_SACNOPOLICY_HPP_
#define INC_ALG_SACNOPOLICY_HPP_
...
public:
	SacNoPolicy(QNetType& qModel1, QNetType& qModel2, QNetType& qTargetModel1, QNetType& qTargetModel2, QOptimizerType& qOpt1, QOptimizerType& qOpt2,
	...
	~SacNoPolicy() = default;
	SacNoPolicy(const SacNoPolicy&) = delete;
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::ReplayBuffer::ReplayBuffer(const int iCap, const at::IntArrayRef& inputShape): cap(iCap) {
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::ReplayBuffer::add(
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
torch::Tensor SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::ReplayBuffer::getSampleIndex(int batchSize) {
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::SacNoPolicy(
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::train(const int epochNum) {
	load();
...
//		LOG4CXX_INFO(logger, "curStateTensor: " << curStateTensor[0][0]);
...
//			torch::Tensor nextLogProbs = torch::log_softmax(nextProbOutput, -1);
			...
			torch::Tensor nextPolicyOutput = (nextQ1 + nextQ2) / 2;
			torch::Tensor nextProbs = torch::softmax(nextPolicyOutput, -1);
			torch::Tensor nextLogProbs = torch::log_softmax(nextPolicyOutput, -1);
...
//			LOG4CXX_INFO(logger, "targetQ: " << targetQ);
		...
		torch::Tensor curEntropy;
		{
			torch::NoGradGuard guard;
			torch::Tensor curPolicyOutput = (curQ1 + curQ2) / 2;
			torch::Tensor curProbs = torch::softmax(curPolicyOutput, -1);
			torch::Tensor curLogProbs = torch::log_softmax(curPolicyOutput, -1);

			curEntropy = ((curProbs * (-1) * curLogProbs)).sum(-1).mean();
			curEntropy = curEntropy.detach();
		}
...
//		torch::Tensor entropy = - (curProbs * curLogProbs).sum(-1);
//		torch::Tensor alphaLoss = (logAlpha * (entropy.detach() - dqnOption.targetEntropy)).mean();
//		torch::Tensor alphaLoss = - torch::mean(logAlpha * (dqnOption.targetEntropy - curEntropy));
		torch::Tensor alphaLoss = - logAlpha * (dqnOption.targetEntropy - curEntropy);
...
//		LOG4CXX_INFO(logger, "loss = " << alphaLoss.item<float>());
		float entropyValue = curEntropy.abs().item<float>();
...
//		LOG4CXX_INFO(logger, "new alpha: " << alpha);
		...
		if ((updateNum % dqnOption.logInterval) == 0) {
			lossStater.update({alpha.item<float>(), entropyValue, alphaLossValue});
		}
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::updateModel(bool force) {
	if (!force) {
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::testModel(const int epochNum, bool render) {
//	std::vector<float> statRewards(dqnOption.envNum, 0);
//	std::vector<float> statLens(dqnOption.envNum, 0);
//
...
//		auto tmp = torch::from_blob(stateVec.data(), testInputShape);
//		LOG4CXX_INFO(logger, "testInputShape: " << testInputShape);
//		LOG4CXX_INFO(logger, "stateVec: " << stateVec);
//		LOG4CXX_INFO(logger, "tmp: " << tmp);
//		auto tmp1 = tmp.div(dqnOption.inputScale);
//		LOG4CXX_INFO(logger, "tmp1: " << tmp1);
//		auto tmp2 = tmp1.to(deviceType);
//		LOG4CXX_INFO(logger, "tmp2: " << tmp2);
		...
		torch::Tensor outputTensor1 = tModel1.forward(inputTensor);
		torch::Tensor outputTensor2 = tModel2.forward(inputTensor);
		torch::Tensor outputTensor = torch::max(outputTensor1, outputTensor2);
		torch::Tensor actionProbs = torch::softmax(outputTensor, -1);
		std::vector<int64_t> actions = policy.getTestActions(actionProbs);
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::test(const int epochNum, bool render) {
	load();
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::save() {
...
template <typename QNetType, typename PNetType, typename EnvType, typename PolicyType, typename QOptimizerType, typename POptimizerType, typename AlphaOptimizerType>
void SacNoPolicy<QNetType, PNetType, EnvType, PolicyType, QOptimizerType, POptimizerType, AlphaOptimizerType>::load() {
