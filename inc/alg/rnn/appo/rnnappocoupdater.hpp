/*
 * rnnappocoupdater.hpp
 *
 *  Created on: Mar 6, 2022
 *      Author: zf
 */

#ifndef INC_ALG_RNN_APPO_RNNAPPOCOUPDATER_HPP_
#define INC_ALG_RNN_APPO_RNNAPPOCOUPDATER_HPP_
...
public:
	RnnAPPOCoUpdater(NetType& behaviorModel, OptimizerType& iOptimizer,
			DqnOption option, AsyncRnnPPODataQ& iq, std::mutex& updateM, int actNum);
	~RnnAPPOCoUpdater() = default;
	RnnAPPOCoUpdater(const RnnAPPOCoUpdater& ) = delete;
...
template<typename NetType, typename OptimizerType>
RnnAPPOCoUpdater<NetType, OptimizerType>::RnnAPPOCoUpdater(NetType& behaviorModel,
...
template<typename NetType, typename OptimizerType>
void RnnAPPOCoUpdater<NetType, OptimizerType>::train(const int updateNum) {
...
//				LOG4CXX_INFO(logger, "ratio is " << ratio);
					kl = ratio.mean().to(torch::kCPU).item<float>();
					...
					if ((updateNum % dqnOption.logInterval) == 0) {
						//print and log
						lossV = lossTensor.item<float>();
						vLossV = valueLossTensor.item<float>();
						aLossV = actLossTensor.item<float>();
						entropyV = entropyTensor.item<float>();
						valueV = valueOutput.mean().item<float>();
						advValue = advTensor.mean().item<float>();
						logValue =  true;
...
//						tLogger.add_scalar("loss/update", updateIndex, (float)updateNum);
				...
				if (logValue) {
					...
					tLogger.add_scalar("loss/adv", updateIndex, advValue);
...
template<typename NetType, typename OptimizerType>
void RnnAPPOCoUpdater<NetType, OptimizerType>::save() {
...
template<typename NetType, typename OptimizerType>
void RnnAPPOCoUpdater<NetType, OptimizerType>::load() {
...
#endif /* INC_ALG_RNN_APPO_RNNAPPOCOUPDATER_HPP_ */
