...
private:
	...
	std::mutex gradMutex;
	std::condition_variable gradCond;
...
void A3CTCPQClientHandle<NetType>::sendGrad() {
	...
	LOG4CXX_INFO(logger, "-------------------------------------> End of sendGrad");
}
...
//			cmdPtr[0] = A3CTCPConfig::GradInterrupt;
			...
			LOG4CXX_DEBUG(logger, "Continue sending grad from " << index);
...
//		sndCmdPtr[0] = A3CTCPConfig::TargetInterrupt;
		...
		LOG4CXX_DEBUG(logger, "Update target with index = " << index << " dataLen = " << dataLen);
...
void A3CTCPQClientHandle<NetType>::completeTarget() {
	...
	try {
		...
		LOG4CXX_DEBUG(logger, "converted to ts: " << ts.size());
...
//	uint64_t cmd = cmdPtr[0];
		...
		LOG4CXX_DEBUG(logger, "---------------------> wants " << len << " remains " << actRemain);


		bufIndex += len;

	LOG4CXX_DEBUG(logger, "Received cmd " << cmd << " len = " << len);
